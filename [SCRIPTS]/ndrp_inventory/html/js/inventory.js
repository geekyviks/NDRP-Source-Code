...
window.addEventListener("message", function (event) {
    if (event.data.action == "display") {
        ...
        $(".nearbyPlayerButton").click(function () {
            ...
            $.post("http://ndrp_inventory/GiveItem", JSON.stringify({
                player: player,
...
function closeInventory() {
    $.post("http://ndrp_inventory/NUIFocusOff", JSON.stringify({}));
}
	...
	$('#itemFast-1').droppable({
        drop: function (event, ui) {
            ...
            if (type === "normal" && (itemInventory === "main" || itemInventory === "fast"))  {
                ...
                $.post("http://ndrp_inventory/PutIntoFast", JSON.stringify({
                    item: itemData,
                    slot : 1
                }));
            }
...
$(document).ready(function () {
    ...
    $('#use').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.usable) {
                ...
                closeInventory();
                $.post("http://ndrp_inventory/UseItem", JSON.stringify({
                    item: itemData
                }));
            }
        }
    ...
    $('#give').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.canRemove) {
                disableInventory(300);
                $.post("http://ndrp_inventory/GetNearPlayers", JSON.stringify({
                    item: itemData
                }));
            }
        }
    ...
    $('#drop').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.canRemove) {
                disableInventory(300);
                $.post("http://ndrp_inventory/DropItem", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
    ...
    $('#playerInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://ndrp_inventory/TakeFromTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://ndrp_inventory/TakeFromProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val()),
					owner : ownerHouse
                }));
                ...
                disableInventory(500);
                $.post("http://ndrp_inventory/TakeFromPlayer", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }else if (type === "normal" && itemInventory === "fast") {
                disableInventory(500);
                $.post("http://ndrp_inventory/TakeFromFast", JSON.stringify({
                    item: itemData
                }));
            } else if (type === "glovebox" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://ndrp_inventory/TakeFromGlovebox", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                })); 
            } else if (type === "shop" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://ndrp_inventory/TakeFromShop", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "disc-property" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://ndrp_inventory/TakeFromDiscProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
    ...
    $('#otherInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://ndrp_inventory/PutIntoTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://ndrp_inventory/PutIntoProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val()),
					owner : ownerHouse
                }));
                ...
                disableInventory(500);
                $.post("http://ndrp_inventory/PutIntoGlovebox", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "disc-property" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://ndrp_inventory/PutIntoDiscProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "player" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://ndrp_inventory/PutIntoPlayer", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
