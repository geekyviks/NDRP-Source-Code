...
admins = {
    'steam:110000112ffafb1',
	'steam:110000134370ace',
	'steam:110000107ea42cd'
...
RegisterCommand('freezetime', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if freezeTime then
                TriggerClientEvent('vSync:notify', source, 'Time is now ~b~frozen.')
            else
    ...
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if not DynamicWeather then
                TriggerClientEvent('vSync:notify', source, 'Dynamic weather changes are now ~r~disabled.')
            else
                TriggerClientEvent('vSync:notify', source, 'Dynamic weather changes are now ~b~enabled.')
            end
        else
            TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^8Error: ^1You are not allowed to use this command.')
        end
...
RegisterCommand('blackout', function(source)
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            blackout = not blackout
            if blackout then
                TriggerClientEvent('vSync:notify', source, 'Blackout is now ~b~enabled.')
            else
                TriggerClientEvent('vSync:notify', source, 'Blackout is now ~r~disabled.')
            end
            TriggerEvent('vSync:requestSync')
        end
    end
