...
Citizen.CreateThread(function()
	...
	while true do
        ...
        local dc = GetDistanceBetweenCoords(pedCoords, 1124.82, -556.13, 55.82, true)
					...
					if cs and cst then
						...
						if IsControlJustReleased(0, 38) and not pressed then
							if not cachedPeds[closestPed] then
								...
								ESX.TriggerServerCallback('ndrp_methsale:hasDrugs', function(hD)
									if hD then hasDrugs = true else hasDrugs = false end
								end)
								
								ESX.TriggerServerCallback('ndrp_base:getpolice', function(hP)
									if hP >= 3 then hasPolice = true else hasPolice = false end
								end)
								...
								if hasDrugs then 
									if hasPolice then 
										...
										exports['mythic_notify']:SendAlert('error', "What's the fun without cops")
										Citizen.Wait(3000)
										pressed = false
									end
								else 
...
TryToSell = function(pedId, coords)
    ...
    if not DoesEntityExist(pedId) or IsPedDeadOrDying(pedId) or IsPedAPlayer(pedId) or IsPedFalling(pedId) then
        Citizen.Trace("ndrp_methsale: ped: " .. pedId .. " not able to sell to.")
        return
    end
...
Sell = function()
    ESX.TriggerServerCallback("ndrp_methsale:sellDrug", function(soldDrug)
        if soldDrug then
