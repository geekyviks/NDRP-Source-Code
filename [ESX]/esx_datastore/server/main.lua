...
MySQL.ready(function()
	local result = exports.ghmattimysql:executeSync('SELECT * FROM datastore')
	for i=1, #result, 1 do
		local name   = result[i].name
		local label  = result[i].label
		local shared = result[i].shared
		local result2 = exports.ghmattimysql:executeSync('SELECT * FROM datastore_data WHERE name = @name', {
			['@name'] = name
		})
		...
		if shared == 0 then
			...
			local data = nil
			if #result2 == 0 then
				exports.ghmattimysql:executeSync('INSERT INTO datastore_data (name, owner, data) VALUES (@name, NULL, \'{}\')', {
					['@name'] = name
				})
				data = {}
			else
...
AddEventHandler('esx:playerLoaded', function(source)
	...
	for i=1, #DataStoresIndex, 1 do
		...
		if dataStore == nil then
			exports.ghmattimysql:execute('INSERT INTO datastore_data (name, owner, data) VALUES (@name, @owner, @data)',
				{
					['@name']  = name,
