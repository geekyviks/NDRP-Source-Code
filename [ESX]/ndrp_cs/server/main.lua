...
TriggerEvent('es:addGroupCommand', 'comserv', 'admin', function(source, args, user)
	if args[1] and GetPlayerName(args[1]) ~= nil and tonumber(args[2]) then
		...
		TriggerClientEvent('chat:addMessage', source, { templateId = 'outlaw', args = { 'Error: ', _U('invalid_player_id_or_actions') } } )
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { templateId = 'outlaw', args = { 'Error: ', _U('insufficient_permissions') } })
...
TriggerEvent('es:addGroupCommand', 'endcomserv', 'admin', function(source, args, user)
	if args[1] then
		if GetPlayerName(args[1]) ~= nil then
			...
			TriggerClientEvent('chat:addMessage', source, { templateId = 'outlaw', args = { 'Error: ', _U('invalid_player_id')  } } )
		end
	else
...
AddEventHandler('esx_communityservice:sendToCommunityService', function(target, actions_count)
	...
	TriggerClientEvent('chat:addMessage', -1, { templateId = 'ooc', args = {'JUDGE: ', _U('comserv_msg', GetPlayerName(target), actions_count) }, color = { 147, 196, 109 } })
...
function releaseFromCommunityService(target)
	...
	MySQL.Async.fetchAll('SELECT * FROM communityservice WHERE identifier = @identifier', {
		...
		if result[1] then
			...
			TriggerClientEvent('chat:addMessage', -1, { templateId = 'ooc', args = { 'JUDGE: ', _U('comserv_finished', GetPlayerName(target)) }, color = { 147, 196, 109 } })
		end
	end)
