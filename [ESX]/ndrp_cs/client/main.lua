...
AddEventHandler('esx_communityservice:inCommunityService', function(actions_remaining)
	...
	while actionsRemaining > 0 and communityServiceFinished ~= true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.ServiceLocation.x, Config.ServiceLocation.y, Config.ServiceLocation.z) > 60 then
			ESX.Game.Teleport(playerPed, Config.ServiceLocation)
				TriggerEvent('chat:addMessage', { templateId = 'outlaw', args = { 'Judge: ', _U('escape_attempt') }, color = { 147, 196, 109 } })
